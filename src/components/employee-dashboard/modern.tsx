...
/**
 * Modern employee dashboard — the current Studio Dumbar kinetic editorial design.
 */
export function EmployeeDashboardModern() {
  const { t } = useT();
  const fmt = useFormat();
      ...
      <section>
        ...
        {upcoming.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-line bg-white p-8 text-center">
            ...
            <p className="mt-2 text-sm font-semibold text-navy-900">{t("e_empty_confirmed")}</p>
            <p className="mt-1 text-xs text-ink/55">{t("e_empty_confirmed_desc")}</p>
          </div>
        ) : (
          <div className="space-y-2">
      ...
      <section className="rounded-2xl border border-line bg-white p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-ink/55">{t("e_certs_desc")}</p>
          </div>
        </div>
