    ...
    <div className="bg-ivory min-h-screen">
      <div className="container-mobile py-5 space-y-10">
        ...
        <section>
          ...
          <p className="mt-3 font-serif italic text-stone-700 leading-snug">
            {upcoming.length === 0
              ? t("e_no_upcoming")
              : upcoming.length === 1
          ...
          <div className="mt-5 flex flex-wrap gap-5 text-sm font-semibold">
            ...
            <Link href="/employee/availability" className="text-stone-700 underline-offset-4 hover:underline">
        ...
        {nextMission && (
          <section className="border-y border-stone-300/60 py-6">
            <div className="text-[10px] font-bold uppercase tracking-[0.32em] text-stone-600">
              / {t("e_next_shift")}
            </div>
            <Link href={`/employee/missions/${nextMission.id}`} className="group mt-4 block">
              <div className="font-serif italic text-stone-700">
              ...
              <dl className="mt-5 grid grid-cols-3 gap-6 border-t border-stone-300/60 pt-5">
                <div>
                  <dt className="text-[10px] font-bold uppercase tracking-[0.32em] text-stone-600">{t("e_label_rate")}</dt>
                  <dd className="mt-1 font-serif text-2xl text-navy-900 tabular">{fmt.euro(nextMission.hourlyRate)}/{t("hours_unit")}</dd>
                </div>
                <div>
                  <dt className="text-[10px] font-bold uppercase tracking-[0.32em] text-stone-600">{t("e_label_earnings")}</dt>
                ...
                <div>
                  <dt className="text-[10px] font-bold uppercase tracking-[0.32em] text-stone-600">{t("e_label_type")}</dt>
        ...
        <section className="grid grid-cols-2 gap-8 border-t border-stone-300/60 pt-6">
          <div>
            <div className="text-[10px] font-bold uppercase tracking-[0.32em] text-stone-600">{t("e_avail_this_week")}</div>
            <div className="mt-3 font-serif text-5xl font-semibold tracking-[-0.02em] text-navy-900 tabular">
              {availableThisWeek}<span className="font-serif italic text-3xl text-stone-500">/7</span>
            </div>
          </div>
          <div>
            <div className="text-[10px] font-bold uppercase tracking-[0.32em] text-stone-600">{t("e_week_label")}</div>
            <div className="mt-3 font-serif text-5xl font-semibold tracking-[-0.02em] text-navy-900 tabular">
              {fmt.hours(weeklyHours)}<span className="font-serif italic text-2xl text-stone-500"> / {contract}{t("hours_unit")}</span>
            </div>
            <div className="mt-2 h-px bg-stone-300/60 relative">
              <span className="absolute left-0 top-0 h-px bg-navy-900" style={{ width: `${pct}%` }} />
            </div>
            <div className="mt-1 text-[11px] uppercase tracking-[0.32em] text-stone-600">{t("e_pct_contract", { n: pct })}</div>
          </div>
        </section>
          ...
          <section className="border-t border-stone-300/60 pt-6">
            ...
            <ul className="divide-y divide-stone-300/60">
              {myInvitations.slice(0, 3).map((rawM) => {
                ...
                return (
                  <li key={m.id}>
                    <Link href={`/employee/missions/${m.id}`} className="group grid grid-cols-12 gap-3 py-5 items-center hover:bg-stone-100/40 -mx-2 px-2">
                      ...
                      <div className="col-span-7 min-w-0">
                        ...
                        <div className="truncate text-xs text-stone-600 mt-0.5">{m.client} · {m.city}</div>
                      </div>
        ...
        <section className="border-t border-stone-300/60 pt-6">
          ...
          {upcoming.length === 0 ? (
            <p className="font-serif italic text-stone-700">{t("e_empty_confirmed")}</p>
          ) : (
            <ul className="divide-y divide-stone-300/60">
        ...
        <section className="border-t border-stone-300/60 pt-6">
          ...
          <p className="mt-2 font-serif italic text-stone-700">{t("e_certs_desc")}</p>
          <ul className="mt-4 grid grid-cols-2 gap-y-2 gap-x-6 border-t border-stone-300/60 pt-4">
